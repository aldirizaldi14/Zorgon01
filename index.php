...
if (isset($_POST["sub"])) {
  ...
  $sql = "SELECT COUNT(*) AS count from users1 where email = :email_id";
  try {
    $stmt = $DB->prepare($sql);
    ...
    if ($result[0]["count"] > 0) {
      ...
      $sql = "INSERT INTO `users1` (`name`, `password`, `email`) VALUES " . "( :name, :pass, :email)";
      $stmt = $DB->prepare($sql);
      ...
      if ($result > 0) {
        ...
        $mail->Host = "mail.bgroup.id";      // sets GMAIL as the SMTP server
        $mail->Port = 465;                   // set the SMTP port for the GMAIL server

        $mail->Username = 'cs@bgroup.id';
        $mail->Password = 'B4yuanggaubaidillah';

        $mail->SetFrom('cs@bgroup.id', 'Bayu Angga');
      ...
      <div class="well contact-form-container">
        <form class="form-horizontal contactform" action="index.php" method="post" name="f" onsubmit="return validateForm();">
          <fieldset>
            ...
            <div class="form-group">
              <div class="col-lg-10">
                <button class="btn btn-primary" href="loginemail.php" onClick="document.location.href='loginemail.php'" name="login">Login Page</button>
              </div>
            </div>
          </fieldset>
        </form>
